...
describe('List', () => {
  ...
  it('#filter', () => {
    const numbers = list.make(3, 4, 5, 8);
  ...
  it('#append', () => {
    const numbers = list.make(3, 4, 5, 8);
    const numbers2 = list.make(3, 2, 9);
    assert.equal(list.toString(list.append(numbers, numbers2)), '(3, 4, 5, 8, 3, 2, 9)');
  });

  it('#isEqual', () => {
    const numbers = list.make(3, 4, 5, 8);
    const numbers2 = list.make(3, 4, 5, 8);
    const numbers3 = list.make(3, 2, 9);
  ...
  it('#isContains', () => {
    const numbers = list.make(3, 4, 5, 8);
    assert.ok(list.isContains(numbers, 3));
    assert.ok(list.isContains(numbers, 8));
    assert.ok(!list.isContains(numbers, 0));
    assert.ok(!list.isContains(numbers, 7));
  });
});
